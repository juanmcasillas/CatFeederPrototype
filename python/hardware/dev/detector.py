...
class DetectorMock(DetectorBase):
    ...
    def GetPhoto(self):
        ...
        shutil.copy('testimage.jpg', photo_path)
...
class DetectorCV2(DetectorBase):
    ...
    def GetPhoto(self):
        ...
        if result:
            ...
            # save the photo to the last photo image
            #cv2.imwrite(photo_path, cv2.cvtColor(img, cv2.COLOR_BAYER_RG2RGB))
    ...
    def Identify(self):
        ...
        while True:
            ...
            if time_delta > IDENTIFY_TIMEOUT:
                logging.info("Can't detect anything in %s seconds. Bailing out" % IDENTIFY_TIMEOUT)
                return None
...
if __name__ == "__main__":
    ...
    if IsRaspberry():
        ...
        camHandler = PiCameraHandler(resolution=(args.camera_width,args.camera_height), framerate=args.camera_fps, rotation=args.camera_rotation,videoport=True)
