...
if __name__ == "__main__":
    ...
    for video in args.input:
        ...
        while True:
            ...
            colors = get_colors(centers, colors)
    ...
    hist,centers = do_KMEANS(np.array(colors), KCOLORS*2)
    ...
    #
    # create cache distance from colors stored in the buffer, to the colors in the histogram, and save it.
    #
    
    #colors = GetUniqueColors(colors)
    #colors_prepared = PrepareMasterColors(np.array(centers))
    #distance_master = kmeans_distance_master(colors, colors_prepared)
