...
class Templater:
    ...
    def tpath(self, tag):
        ...
        h = os.sep.join( bd + [  self.tplpart % (tag,'head') ])
        b = os.sep.join( bd + [  self.tplpart % (tag,'body') ])
    ...
    def Generate(self, ftpl, xtable, opt=None):

        hf, bf, ff = self.tpath(ftpl)
